/*
 * $Id: MultiFiles.cpp 1886 2010-05-16 19:28:57Z kinddragon $
 *
...
ULONGLONG CMultiFiles::Seek(LONGLONG lOff, UINT nFrom)
{
	...
	else
	{
		...
		while ((LONGLONG)(m_FilesSize[nNewPart]+llSum) <= lAbsolutePos)
		{
...
UINT CMultiFiles::Read(void* lpBuf, UINT nCount)
{
	...
	do
	{
		...
		if (dwRead != nCount && (size_t)m_nCurPart < m_strFiles.GetCount()-1)
		{
			OpenPart (m_nCurPart+1);
			lpBuf	 = (void*)((BYTE*)lpBuf + dwRead);
			nCount  -= dwRead;
		}
	} while (nCount != dwRead && (size_t)m_nCurPart < m_strFiles.GetCount()-1);
	return dwRead;
}
